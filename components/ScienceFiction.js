...
export default function Library({ navigation }) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get('https://www.googleapis.com/books/v1/volumes?q=subject:science+fiction');
      ...
      <ImageBackground source={require('./images/bg.png')} style={styles.bg}>
        ...
        <View>
          <ScrollView style={styles.MainOption} horizontal={true} showsHorizontalScrollIndicator={false}>
            <TouchableOpacity style={styles.MainOptionTopicsLive} onPress={()=>navigation.navigate('ScienceFiction')}>
              <Text style={styles.MainOptionTopicsTextLive}>Science Fiction</Text>
            </TouchableOpacity>
