...
export default function Library({ navigation }) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get('https://www.googleapis.com/books/v1/volumes?q=subject:health&fitness');
        ...
        <View>
          <ScrollView style={styles.MainOption} horizontal={true} showsHorizontalScrollIndicator={false}>
            <TouchableOpacity style={styles.MainOptionTopicsLive} onPress={()=>navigation.navigate('Thriller')}>
              <Text style={styles.MainOptionTopicsTextLive}>Health & Fitness</Text>
            </TouchableOpacity>
