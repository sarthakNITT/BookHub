...
export default function Library({ navigation }) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get('https://www.googleapis.com/books/v1/volumes?q=subject:sports');
      ...
      <ImageBackground source={require('./images/bg.png')} style={styles.bg}>
        ...
        <View>
          <ScrollView style={styles.MainOption} horizontal={true} showsHorizontalScrollIndicator={false}>
            <TouchableOpacity style={styles.MainOptionTopicsLive} onPress={()=>navigation.navigate('Sports')}>
              <Text style={styles.MainOptionTopicsTextLive}>Sports</Text>
            </TouchableOpacity>
